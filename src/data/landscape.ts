...
export const INITIAL_NODES: AnyNode[] = [
  ...
  // ---------------------------------------------------------------------------
  // June 2026 ADDITIONS — auto-update sweep
  //   • P-EGNN — probabilistic equivariant-GNN MLIP via learned functional
  //     perturbations (uncertainty-aware), with P-Orb as the Orb-v3 finetune
  // ---------------------------------------------------------------------------

  {
    ...
    y: 150,
    desc:
      "Probabilistic equivariant-GNN MLIP built on the EGNN backbone, augmented with the P-MLIP recipe: a lightweight plug-in module that introduces learned functional perturbations into the network's parameter space and is finetuned end-to-end with the Continuous Ranked Probability Score (CRPS), a proper scoring rule. Position-update networks are left noise-free so that E(n)-equivariance is preserved exactly. The P-MLIP framework is also applied to the Orb-v3 foundation model (the resulting model is called P-Orb), demonstrating that the same recipe upgrades both from-scratch and pretrained MLIPs into uncertainty-aware probabilistic potentials. On the N-body charged-particle benchmark P-EGNN improves CRPS over the Bayesian MLIP baseline BLIP by 19–32% across all training-set sizes, and on the silica-glass (SiO2, 699 atoms) benchmark P-Orb raises the Spearman correlation between predicted uncertainty and actual error from 0.75 (BLIP-Orb) to 0.84 — pushing the Pareto frontier for non-ensemble uncertainty quantification in MLIPs.",
    paperUrl: "https://arxiv.org/abs/2605.19939",
    isNew: true,
    coverage: ["small molecules", "amorphous oxides", "silica glass", "N-body charged particles"],
    useCases: ["uncertainty quantification", "active learning", "error-aware MD", "probabilistic foundation-model finetuning"],
    properties: ["energy", "forces"],
    frameworks: ["PyTorch"],
    maintenance: "experimental",
    lastReviewed: "2026-06-07",
    trainingData: ["N-body charged-particle benchmark", "silica-glass (SiO2) DFT"],
    tags: ["equivariant", "EGNN", "probabilistic", "uncertainty quantification", "learned perturbations", "CRPS", "P-MLIP"],
    supportsCharges: false,
    supportsSpins: false,
    elementsCovered: "dataset-dependent (small molecules and silica)",
    equivariance: "constrained",
    architecture: "gnn",
    usesAttention: false,
    longRange: false,
...
export const INITIAL_EDGES: Edge[] = [
  ...
  { from: "painn", to: "hydragnn", label: "PaiNN backbone", description: "HydraGNN-GFM uses a PaiNN-based equivariant scalar/vector message-passing backbone selected via DeepHyper hyperparameter optimization on Frontier; PaiNN supplies the underlying interaction representation around which the multi-task heads are organised." },
  { from: "uma", to: "hydragnn", label: "Multi-task foundation", dashed: true, description: "HydraGNN-GFM is a multi-task atomistic foundation model jointly trained on 16 first-principles datasets covering 85+ elements, sitting in the same multi-domain foundation-model niche that UMA established for atomistic simulation." },
  ...
  { from: "aimnet_nse", to: "qnep", label: "Charge-aware sibling", dashed: true, description: "Sibling charge-aware MLIP design: qNEP injects environment-dependent partial charges into the NEP descriptor backbone with explicit electrostatics, while AIMNet-NSE injects total charge / spin states into a message-passing backbone via its Neural Spin Equilibration loop." },

  // March 2026 — PET-MAD-1.5 (102-element r²SCAN universal PET potential trained on MAD-1.5)
  { from: "petmad", to: "petmad15", label: "MAD → MAD-1.5 / r²SCAN", description: "PET-MAD-1.5 is the direct successor to PET-MAD from the same EPFL lab-cosmo group: same Point Edge Transformer backbone, retrained at the r²SCAN level on the curated MAD-1.5 dataset that extends elemental coverage from 85 to 102 elements with targeted enrichment of molecules, clusters, surfaces, and low-dimensional structures." },
  { from: "mace_osaka26", to: "petmad15", label: "Broad-element uMLIP", dashed: true, description: "Sibling broad-element universal potentials: MACE-Osaka26 reaches 97 elements within the MACE equivariant family; PET-MAD-1.5 reaches 102 elements within the unconstrained PET transformer family. The link captures shared coverage-of-the-periodic-table goals rather than architectural descent." },
  ...
  { from: "petmad", to: "pet_oam_xl", label: "OAM training mix", description: "PET-OAM-XL keeps the unconstrained Point Edge Transformer backbone introduced by PET / PET-MAD and scales it to extra-large capacity, retraining on the OMat24 + sAlex + MPtrj (OAM) data mixture to top the Matbench Discovery leaderboard." },
  { from: "petmad15", to: "pet_oam_xl", label: "PET family", dashed: true, description: "Sibling 2026 PET releases from the EPFL lab-cosmo group: PET-MAD-1.5 emphasises broad elemental coverage at r²SCAN level, while PET-OAM-XL emphasises raw stability-prediction accuracy via OMat24 pre-training and is positioned as the Matbench Discovery reference model." },

  // May 2026 — AlphaNet (local-frame-based equivariant MLIP)
  { from: "nequip", to: "alphanet", label: "Local frames", description: "AlphaNet swaps the spherical-harmonic tensor-product machinery used by NequIP-style E(3)-equivariant MLIPs for atom-centred local frames with learnable geometric transitions, retaining equivariance while reducing per-step cost." },
  { from: "mace", to: "alphanet", label: "Frame vs CG tensors", dashed: true, description: "AlphaNet targets the same Matbench Discovery / OC2M foundation niche as MACE-style equivariant potentials, replacing Clebsch-Gordan tensor products with cheaper local-frame operations to improve scalability." },
  ...
  { from: "grace", to: "tace", label: "Foundation-scale ACE", dashed: true, description: "Sibling foundation-scale ACE-family MLIPs: GRACE realises a graph implementation of spherical-harmonic ACE while TACE realises an irreducible-Cartesian-tensor implementation of ACE, both released as universal potentials on the Matbench Discovery OAM track." },
  { from: "mace_polar1", to: "tace", label: "Tensorial properties", dashed: true, description: "Like MACE-POLAR-1, TACE exposes tensorial electronic observables (charges, magnetic moments, external-field response) on top of an ACE-family backbone, additionally including a Latent Ewald Summation module for explicit long-range electrostatics." },
  ...
  { from: "dtnn", to: "schnet", label: "Continuous filters", description: "SchNet is the direct successor to DTNN from the same Schütt/Tkatchenko/Müller group, replacing DTNN's tensorised pairwise interaction blocks with continuous-filter convolutions while keeping the per-element embedding and energy-decomposition framework." },

  // CGCNN (2018) — foundational crystal-graph network
  { from: "cgcnn", to: "megnet", label: "Crystal Graph", description: "MEGNet generalises the CGCNN crystal-graph framework to molecules and crystals jointly with global state attributes, sitting in the direct CGCNN crystal-graph lineage." },
  { from: "cgcnn", to: "alignn", label: "+Line Graph", dashed: true, description: "ALIGNN extends the CGCNN crystal-graph idea by augmenting the atomic graph with a line graph so bond angles become first-class nodes, while preserving the CGCNN-style multi-edge crystal-graph backbone." },
  { from: "cgcnn", to: "gnome", label: "Crystal-graph MLIP", dashed: true, description: "GNoME's discovery pipeline relies on graph-network MLIPs trained on crystal graphs in the CGCNN tradition, with active learning closing the loop between DFT and graph-network energy prediction." },

  // MEGNet (2019) — Materials Virtual Lab predecessor to M3GNet
  { from: "megnet", to: "m3gnet", label: "+3-body", description: "M3GNet from the same Materials Virtual Lab group extends MEGNet's joint molecule/crystal graph network with explicit three-body interactions and stress prediction for MLIPs." },

  // PhysNet (2019) — charge-aware molecular potential, precursor to SpookyNet
  { from: "physnet", to: "spookynet", label: "Charge/Dipole", description: "SpookyNet inherits PhysNet's modular charge/dipole/electrostatic energy decomposition (both from collaborators in the Unke/Müller circle) and adds self-attention with explicit total-charge and spin-multiplicity tokens." },
  { from: "physnet", to: "aimnet", label: "Charge-aware MLIPs", dashed: true, description: "PhysNet and AIMNet developed concurrently as charge-aware molecular potentials with learnable atomic charges plus Coulomb correction; sibling architectures rather than direct descendants." },

  // Cormorant (2019) — early end-to-end SO(3)-equivariant network
  { from: "cormorant", to: "nequip", label: "Equivariant precursor", dashed: true, description: "Cormorant pioneered end-to-end SO(3)-equivariant neural networks using Clebsch-Gordan tensor products on irreducible representations; NequIP applies the same equivariant tensor-product machinery to interatomic potentials." },
  { from: "tfn", to: "cormorant", label: "Parallel SO(3)", dashed: true, description: "Cormorant and Tensor Field Networks were developed in parallel in 2018-2019 as the two early frameworks for SO(3)-equivariant point-cloud/molecular networks using spherical harmonics and Clebsch-Gordan tensor products." },

  // EGNN (2021) — simple E(n)-equivariant baseline
  { from: "painn", to: "egnn", label: "Scalar/vector", dashed: true, description: "EGNN and PaiNN were proposed concurrently (both ICML 2021) as lightweight scalar/vector equivariant message-passing alternatives to higher-order tensor methods; sibling architectures rather than direct descendants." },

  // NEP (2021) — original Neuroevolution Potential
  { from: "bpnn", to: "nep_orig", label: "HDNNP + NES", description: "NEP sits in the Behler-Parrinello high-dimensional NN potential family — per-element neural networks on local descriptors — but replaces gradient-based training with a separable natural-evolution strategy for raw GPU throughput in GPUMD." },
  { from: "nep_orig", to: "nep89", label: "+89 elements", description: "NEP89 is a direct universal extension of the original NEP framework, scaling the same NEP descriptor + neuroevolution training recipe to 89-element coverage with a curated DFT dataset." },
  { from: "nep_orig", to: "qnep", label: "+Dynamic charges", dashed: true, description: "qNEP extends the NEP framework with environment-dependent partial charges and explicit Ewald electrostatics, building directly on the NEP descriptor backbone introduced by Fan et al. in 2021." },
  { from: "nep_orig", to: "orion", label: "Organic NEP", dashed: true, description: "ORION is an organic-chemistry universal force field built within the NEP framework that the original NEP paper introduced." },

  // PFP (2022) — first universal NNP
  { from: "pfp_orig", to: "pfp_v8", label: "v1 → v8", description: "PFP v8 is the eighth generation of the Preferred Potential, descending directly from the original PFP introduced in 2022 with successive accuracy / coverage / speed improvements and a switch to r2SCAN reference data." },
  { from: "m3gnet", to: "pfp_orig", label: "Universal NNP", dashed: true, description: "Sibling first-generation universal NNPs released around 2022: PFP and M3GNet were the two earliest universal MLIPs covering >40 elements, developed in parallel for industrial (PFP) and academic (M3GNet) deployment." },
  { from: "pfp_orig", to: "mattersim", label: "Universal NNP family", dashed: true, description: "MatterSim from Microsoft sits in the same universal-NNP niche that PFP established as the first commercial universal MLIP; the link captures shared positioning rather than architectural descent." },

  // Equiformer V1 (2022) — direct ancestor of Equiformer V2
  { from: "tfn", to: "equiformer_v1", label: "+Attention", description: "Equiformer V1 combines graph attention with TFN-style E(3)-equivariant tensor representations and depthwise tensor-product MLPs; explicit successor to TFN-style equivariant convolutions with attention added." },
  { from: "equiformer_v1", to: "eqv2", label: "v1 → v2", description: "Equiformer V2 is the direct higher-degree successor to Equiformer V1 from the same MIT Atomic Architects / Meta FAIR collaboration, scaling the equivariant attention to higher-degree spherical representations using eSCN convolutions." },

  // SCN / eSCN (2022/2023) — Meta FAIR Spherical Channels lineage
  { from: "scn", to: "escn", label: "SO(3)→SO(2)", description: "eSCN is a direct refinement of SCN that reduces SO(3) tensor-product convolutions to SO(2) by aligning each pair to a common rotation axis, dramatically lowering the cost of high-degree equivariant convolutions." },
  { from: "escn", to: "eqv2", label: "Backbone", description: "Equiformer V2 explicitly uses eSCN convolutions as its high-degree spherical convolutional backbone, citing eSCN as the convolution operator that makes higher-degree representations efficient." },
  { from: "gemnet", to: "scn", label: "OC20 lineage", dashed: true, description: "SCN succeeds GemNet-OC as Meta FAIR's flagship OC20 model, replacing GemNet-OC's spherical message passing with multichannel spherical signal representations on edge-aligned local frames." },

  // SO3krates (2022) — equivariant attention for arbitrary length scales
  { from: "tfn", to: "so3krates", label: "+Attention scales", description: "SO3krates extends TFN-style SO(3)-equivariant tensor-product machinery with invariant scalar attention plus an equivariant filter, enabling long-range-capable equivariant transformers." },
  { from: "spookynet", to: "so3krates", label: "Long-range MLIPs", dashed: true, description: "SO3krates and SpookyNet share an emphasis on long-range / electronic-structure-aware MLIPs; SpookyNet uses self-attention with charge/spin tokens, SO3krates uses equivariant attention factorised across length scales. Both contributed to the long-range MLIP design space." },
  ...
  { from: "mace_mp0", to: "mace_polar1", label: "Adds polarisation", dashed: true, description: "MACE-POLAR-1 extends the MACE foundation-model line with non-self-consistent polarisable charge/spin densities and Fukui equilibration; descends from the MACE-MP family rather than the bare MACE architecture." },

  // MACE-OFF (2023) — organic foundation MACE
  { from: "mace", to: "mace_off", label: "Organic FF", description: "MACE-OFF is the organic-chemistry sibling of MACE-MP-0: the MACE architecture trained on SPICE quantum-chemistry data for transferable molecular dynamics across H, C, N, O, F, P, S, Cl, Br, I." },
  { from: "mace_mp0", to: "mace_off", label: "MACE foundation pair", dashed: true, description: "MACE-MP-0 (materials, MPtrj, 89 elements) and MACE-OFF (organic, SPICE, 10 elements) are sibling MACE foundation models released by overlapping author teams as the materials and organic universal MACE potentials." },
  { from: "mace_off", to: "grace_off", label: "Organic foundation analog", dashed: true, description: "GRACE-OFF is the GRACE-architecture analog of MACE-OFF for organic liquids — a direct conceptual sibling using the same SPICE-style organic universal-MLIP recipe within the GRACE rather than MACE family." },

  // GNoME (2023) — DeepMind universal materials GNN
  { from: "m3gnet", to: "gnome", label: "Universal materials GNN", dashed: true, description: "GNoME and M3GNet are sibling universal materials GNNs from 2022-2023; both target broad-coverage stable-structure prediction, with GNoME scaling active-learning-coupled NNP-driven discovery to 2.2M new crystal structures (380k stable)." },
  { from: "gnome", to: "mattersim", label: "Materials discovery", dashed: true, description: "MatterSim follows GNoME in the universal materials-discovery foundation-model niche; both leverage active-learning-coupled NNP-driven exploration of crystal-structure space at scale." },

  // MPNICE (2025) — Schrödinger charge-aware invariant MP MLIP
  { from: "aimnet2", to: "mpnice", label: "Charge-aware MP", description: "MPNICE extends the charge-aware message-passing MLIP design popularised by AIMNet2 — invariant local messaging with an explicit iterative charge-equilibration step and long-range Coulomb electrostatics — to an 89-element pretrained model family from Schrödinger." },
  { from: "bpnn", to: "mpnice", label: "HDNNP + Qeq", dashed: true, description: "MPNICE sits in the Behler-Parrinello high-dimensional NN potential tradition (atomic energy decomposition + local descriptors) and adds an iterative charge-equilibration (Qeq) head and explicit long-range Coulomb electrostatics on top." },
  ...
  { from: "equiewald", to: "lorem", label: "Long-range equivariant MLIP", dashed: true, description: "Sibling long-range equivariant MLIPs: EquiEwald embeds an Ewald-inspired reciprocal-space formulation inside SO(3)-equivariant message passing, while LOREM keeps the message-passing in real space but promotes the long-range carrier from scalar to equivariant charges. Both target orientation-dependent long-range interactions on top of the NequIP equivariant lineage." },
  { from: "petmad", to: "lorem", label: "lab-cosmo", dashed: true, description: "Same EPFL lab-cosmo group as the PET / PET-MAD line; LOREM is the lab-cosmo entry into long-range-aware equivariant message-passing MLIPs, complementary to the unconstrained PET transformer backbone." },
  ...
  // 2026 — P-EGNN (probabilistic equivariant-GNN MLIP via learned functional perturbations)
  { from: "egnn", to: "p_egnn", label: "+Learned perturbations", description: "P-EGNN keeps the E(n)-equivariant EGNN backbone unchanged and wraps it with the P-MLIP recipe: learned functional perturbations injected into the parameter space plus end-to-end finetuning under the Continuous Ranked Probability Score, turning the deterministic EGNN into an uncertainty-aware probabilistic MLIP while leaving position-update networks noise-free so that E(n)-equivariance is preserved exactly." },
  { from: "orb", to: "p_egnn", label: "P-Orb finetune", dashed: true, description: "The P-MLIP recipe behind P-EGNN is also demonstrated by finetuning the Orb-v3 foundation model into a probabilistic variant called P-Orb, which on the silica-glass benchmark raises the Spearman correlation between predicted uncertainty and actual error from 0.75 (BLIP-Orb) to 0.84." },
];
